...
def test_read_players_by_name(client):
...
def test_read_players_with_id(client):
...
def test_read_performances_by_date(client):
    response = client.get(
...
def test_read_leagues_with_id(client):
...
# test /v0/teams/
def test_read_teams_for_one_league(client):
